...
#include "../Commands/StopSending.h"
...
void DataSending::SendTheData(bool sending){
	while(sending){
	...
	char line4[100];
	char line5[100];
	string gearSetting = "Shifted to "+Robot::shifterSystem->GetGearSetting();
	strcpy(line1,gearSetting.c_str());
	string driveSetting = "Drive Mode "+Robot::driveTrain->GetDriveMode();
	strcpy(line2,driveSetting.c_str());
	...
	sprintf(line4, "Battery Current usage: %f A",batteryCurrent->GetVoltage());
	sprintf(line5, "%f Degrees",RobotMap::collectorAngleAdjuster->GetAngle());
	...
	Send(downTime,4);
	Send(percentCPU,4);
	Send(((RobotMap::collectorElevationEncoder->GetVoltage()*(1024.0/5.0))/2.54));
